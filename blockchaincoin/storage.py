...
class StoredBlock:
    """Legacy account-ledger block plus cumulative work."""
...
def _decode_consensus_block(payload: str) -> ConsensusBlock:
    try:
        ...
        return ConsensusBlock.from_dict(json.loads(payload))


class SQLiteBlockStore:
    """Durable block storage for a single node.

    The store uses SQLite WAL mode so writes are atomic and readers can inspect
    the chain while another process is appending blocks.
    """
    ...
    def put_block(self, block: Block) -> StoredBlock:
        if self.has_block(block.hash):
            existing = self.get_block(block.hash)
            return existing

        tip = self.tip()
        if tip is None:
            if block.index != 0 or block.previous_hash != "0" * 64:
                raise StorageError("first stored block must be genesis")
            cumulative_work = self._work_for(block)
        else:
            if block.index != tip.block.index + 1:
                raise StorageError("block height does not extend current tip")
            if block.previous_hash != tip.block.hash:
                raise StorageError("block previous hash does not match current tip")
            cumulative_work = tip.cumulative_work + self._work_for(block)
        ...
        return StoredBlock(block=block, cumulative_work=cumulative_work)
    ...
    def get_block(self, block_hash: str) -> StoredBlock:
        row = self.connection.execute(
            "SELECT data, cumulative_work FROM blocks WHERE hash = ?",
            (block_hash,),
        ).fetchone()
        if row is None:
    ...
    def block_at_height(self, height: int) -> StoredBlock:
        row = self.connection.execute(
            "SELECT hash FROM blocks WHERE height = ?",
            (height,),
        ).fetchone()
        if row is None:
            raise StorageError(f"unknown block height: {height}")
        return self.get_block(str(row["hash"]))

    def tip(self) -> StoredBlock | None:
        row = self.connection.execute(
    ...
    def iter_blocks(self) -> Iterator[StoredBlock]:
        rows = self.connection.execute("SELECT hash FROM blocks ORDER BY height ASC").fetchall()
        for row in rows:
            yield self.get_block(str(row["hash"]))

    @staticmethod
    def _work_for(block: Block) -> int:
        return 2**block.difficulty


class SQLiteConsensusStore:
    """Durable storage for UTXO consensus blocks."""
    ...
    def put_block(self, block: ConsensusBlock) -> StoredConsensusBlock:
        ...
        tip = self.tip()
        if tip is None:
            if block.height != 0 or block.previous_hash != "0" * 64:
                raise StorageError("first stored block must be genesis")
            cumulative_work = self._work_for(block)
        else:
            if block.height != tip.block.height + 1:
                raise StorageError("block height does not extend current tip")
            if block.previous_hash != tip.block.hash:
                raise StorageError("block previous hash does not match current tip")
            cumulative_work = tip.cumulative_work + self._work_for(block)
    ...
    def has_block(self, block_hash: str) -> bool:
        row = self.connection.execute(
            "SELECT 1 FROM consensus_blocks WHERE hash = ?",
            (block_hash,),
        ).fetchone()
        return row is not None
    ...
    def get_block(self, block_hash: str) -> StoredConsensusBlock:
        row = self.connection.execute(
            "SELECT data, cumulative_work FROM consensus_blocks WHERE hash = ?",
            (block_hash,),
        ).fetchone()
        if row is None:
            raise StorageError(f"unknown block: {block_hash}")
    ...
    def block_at_height(self, height: int) -> StoredConsensusBlock:
        row = self.connection.execute(
            "SELECT hash FROM consensus_blocks WHERE height = ?",
            (height,),
        ).fetchone()
        if row is None:
            raise StorageError(f"unknown block height: {height}")
    ...
    def tip(self) -> StoredConsensusBlock | None:
        row = self.connection.execute(
            "SELECT hash FROM consensus_blocks ORDER BY height DESC LIMIT 1"
        ).fetchone()
        if row is None:
            return None
        return self.get_block(str(row["hash"]))
    ...
    def iter_blocks(self) -> Iterator[StoredConsensusBlock]:
        rows = self.connection.execute(
            "SELECT hash FROM consensus_blocks ORDER BY height ASC"
        ).fetchall()
        for row in rows:
            yield self.get_block(str(row["hash"]))
...
class SQLiteMempoolStore:
    ...
    def _decode(payload: str) -> ConsensusTransaction:
        try:
            ...
            return ConsensusTransaction.from_dict(json.loads(payload))

    ...
    def test_wallet_to_wallet_utxo_lifecycle_through_application_entrypoint(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            alice_status = self.run_json(
                "utxo-status", "--db", str(node_db), "--wallet", str(alice_wallet)
            )
            bob_status = self.run_json(
                "utxo-status", "--db", str(node_db), "--wallet", str(bob_wallet)
            )
            ...
            alice_utxos = self.run_json(
                "utxo-utxos", "--db", str(node_db), "--wallet", str(alice_wallet)
            )
            ...
            mempool = self.run_json("utxo-mempool", "--db", str(node_db))
            ...
            pending_tx = self.run_json("utxo-tx", "--db", str(node_db), "--txid", txid)
            self.assertEqual(pending_tx["status"], "mempool")
            self.assertEqual(pending_tx["output_value"], 97)

            pruned = self.run_json("utxo-mempool", "--db", str(node_db), "--prune")
            self.assertEqual(pruned["removed"], 0)

            mined_text = self.run_app(
                "utxo-mine", "--db", str(node_db), "--miner-wallet", str(miner_wallet)
            )
            ...
            final_bob = self.run_json(
                "utxo-status", "--db", str(node_db), "--wallet", str(bob_wallet)
            )
            final_miner = self.run_json(
                "utxo-status", "--db", str(node_db), "--wallet", str(miner_wallet)
            )
            ...
            block = self.run_json("utxo-block", "--db", str(node_db), "--height", "1")
            self.assertTrue(block["active"])
            self.assertEqual(block["transaction_count"], 2)
            self.assertIn(txid, block["transactions"])

            confirmed_tx = self.run_json("utxo-tx", "--db", str(node_db), "--txid", txid)
            ...
            bob_utxos = self.run_json(
                "utxo-utxos", "--db", str(node_db), "--wallet", str(bob_wallet)
            )
            self.assertEqual(bob_utxos["balance"], 37)
            self.assertEqual(bob_utxos["count"], 1)
            self.assertEqual(bob_utxos["utxos"][0]["amount"], 37)

            empty_mempool = self.run_app("utxo-mempool", "--db", str(node_db))
